	...
	void Start()
	{
		...
		if(Physics.Raycast(ray, out hit, Range, ~((1 << vp_Layer.Player) | (1 << vp_Layer.Debris))))
		{
			...
			if (m_ImpactSounds.Count > 0)
			{
				...
				GetComponent<AudioSource>().pitch = Random.Range(SoundImpactPitch.x, SoundImpactPitch.y);
				GetComponent<AudioSource>().PlayOneShot(m_ImpactSounds[(int)Random.Range(0, (m_ImpactSounds.Count))]);
			}
