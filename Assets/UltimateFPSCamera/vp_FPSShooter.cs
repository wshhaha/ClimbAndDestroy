...
public class vp_FPSShooter : vp_Component
{
	...
	public void Fire()
	{
		...
		{
			GetComponent<AudioSource>().pitch = Random.Range(SoundFirePitch.x, SoundFirePitch.y);
	...
	public void DryFire()
	{
		...
		if (GetComponent<AudioSource>() != null)
		{
			...
			GetComponent<AudioSource>().PlayOneShot(SoundDryFire);
	...
	protected void EjectShell()
	{
		...
		if (s.GetComponent<Rigidbody>())
			s.GetComponent<Rigidbody>().AddForce((transform.TransformDirection(ShellEjectDirection) * ShellEjectVelocity), ForceMode.Impulse);
