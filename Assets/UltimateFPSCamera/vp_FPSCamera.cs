...
public class vp_FPSCamera : vp_Component
{
	...
	protected new void Awake()
	{
		...
		GetComponent<Camera>().cullingMask &= ~((1 << vp_Layer.Player) | (1 << vp_Layer.Weapon));
	...
	protected void LateUpdate()
	{
		...
		gameObject.GetComponent<Camera>().transform.rotation = xQuaternion * yQuaternion;

		// roll the camera
		gameObject.GetComponent<Camera>().transform.localEulerAngles += new Vector3(0, 0, m_RotationSpring.State.z);
	...
	protected void UpdateZoom()
	{
		...
		gameObject.GetComponent<Camera>().fieldOfView = Mathf.SmoothStep(gameObject.GetComponent<Camera>().fieldOfView, RenderingFieldOfView, zoom);
	...
	public void SwitchWeapon(int weapon, WeaponSwitchDelegate callback = null)
	{
		...
		if (CurrentWeapon != null)
		{
			...
			// play unwield sound
			if (CurrentWeapon.GetComponent<AudioSource>() != null)
			{
				if (CurrentWeapon.SoundUnWield != null)
				{
					CurrentWeapon.GetComponent<AudioSource>().pitch = 1;
					CurrentWeapon.GetComponent<AudioSource>().PlayOneShot(CurrentWeapon.SoundUnWield);
				}
			}
			...
			m_ShowWeaponTimer = vp_Timer.In(0.15f, delegate()
			{
				if (CurrentWeapon != null)
				{
					...
					if (CurrentWeapon.GetComponent<AudioSource>() != null)
					{
						...
						{
							CurrentWeapon.GetComponent<AudioSource>().pitch = 1;
							if (CurrentWeapon.GetComponent<AudioSource>().gameObject.active)
								CurrentWeapon.GetComponent<AudioSource>().PlayOneShot(CurrentWeapon.SoundWield);
						}
					}
				}
	...
	public void SetWeapon(int i)
	{
		...
		if (m_CurrentWeaponID > 0)
		{
			...
			if (m_CurrentWeapon != null)
			{
				...
				if (m_CurrentWeapon.gameObject.GetComponent<Renderer>() != null)
					m_CurrentWeapon.gameObject.GetComponent<Renderer>().enabled = true;
