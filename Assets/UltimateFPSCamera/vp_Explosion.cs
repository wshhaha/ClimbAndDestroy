...
public class vp_Explosion : MonoBehaviour
{
	...
	void Awake()
	{
		...
		foreach (Collider hit in colliders)
		{
			if (hit != this.GetComponent<Collider>())
			{
				...
				if (hit.GetComponent<Rigidbody>())
				{
					...
					hit.GetComponent<Rigidbody>().AddExplosionForce(Force, transform.position, Radius, UpForce);
		...
		GetComponent<AudioSource>().dopplerLevel = 2.5f;
		GetComponent<AudioSource>().pitch = Random.Range(SoundMinPitch, SoundMaxPitch);
		GetComponent<AudioSource>().playOnAwake = false;	// 'playOnAwake' seems unreliable, so play manually
