...
public class XffectEventCustom : Editor 
{
	...
	protected void RadialBlurConfig(XftEventComponent ctarget)
	{
		...
		if (DisplayRadialBlurConfig)
        {
			...
			ctarget.RBSampleDist = EditorGUILayout.Slider("sample dist:",ctarget.RBSampleDist,0f,1f);
			...
			if (ctarget.RBStrengthType == MAGTYPE.Fixed)
				ctarget.RBSampleStrength = EditorGUILayout.FloatField("sample strength:",ctarget.RBSampleStrength);
			else
